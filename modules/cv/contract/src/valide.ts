 ...
 * Validation du CV.
 *
 * Le CV est aujourd'hui un fichier JSON versionné : le compilateur ne le relit pas, et une
...
class Contrôle {
  ...
  /** Tableau obligatoire et non vide : une section vide ne se justifie pas sur un CV. */
...
/**
 * Vérifie qu'une valeur inconnue est un CV exploitable.
 *
 * Ne renvoie jamais d'exception : l'appelant décide quoi faire de l'échec (503 côté API,
 * sortie non nulle côté CI).
 */
export function valideCv(valeur: unknown): Resultat {
  const c = new Contrôle();

  if (!c.objet(valeur, 'cv')) return { ok: false, erreurs: c.erreurs };
  ...
  return c.erreurs.length === 0
    ? { ok: true, cv: valeur as unknown as Cv }
