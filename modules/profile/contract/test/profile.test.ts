/**
 * Le profil est le seul contenu du site : s'il est cassé, il n'y a rien à afficher. Ces tests sont
...
import { lireProfile, valideProfile } from '../src/index.js';

test('le profil versionné dans le dépôt est valide', () => {
  const resultat = lireProfile();
  // En cas d'échec, on veut lire *quelles* règles ont sauté, pas un simple « false ».
  assert.ok(resultat.ok, `Profil invalide :\n${resultat.ok ? '' : resultat.erreurs.join('\n')}`);
});
...
test('les expériences sont rangées de la plus récente à la plus ancienne', () => {
  ...
  const débuts = resultat.profile.experiences.map((e) => rang(e.debut));
...
test("une valeur qui n'est pas un objet est rejetée sans lever d'exception", () => {
  for (const valeur of [null, undefined, 42, 'texte', []]) {
    const resultat = valideProfile(valeur);
    assert.equal(
      resultat.ok,
      false,
...
/** Un profil minimal valide, que chaque test déforme sur un seul point. */
function squelette() {
  return {
    profil: {
      nom: 'Nathan SENG',
